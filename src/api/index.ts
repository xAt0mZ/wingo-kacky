...
import { Edition, FAV_SHEET, PoulesSheets, Sheet, SheetRanges } from '../models/consts';
...
const pouleRanges = Object.values(Edition).map((edition) => PoulesSheets[edition] ? `ranges=${PoulesSheets[edition]}!A:D` : '').join('&');
const favRange = `ranges=${FAV_SHEET}!A2:B76`;
const options = 'valueRenderOption=FORMULA&dateTimeRenderOption=FORMATTED_STRING';
const url = `${baseURL}?${key}&${ranges}&${pouleRanges}&${favRange}&${options}`;
